...
while true; do
    ...
    if grep "$disk_chk" devices; then
        ...
        if [ "$auto_prov_ans" = "y" ]; then
            ...
            parted -a optimal $disk_chk --script mkpart primary 131MiB -1
            parted $disk_chk --script set 2 lvm on
            part_1=("${disk_chk}p1")
            part_2=("${disk_chk}p2")
            part_3=("${disk_chk}p3")
            ...
            cryptsetup --verbose --type luks1 --cipher serpent-xts-plain64 --key-size 512 --hash whirlpool --iter-time 10000 --use-random --verify-passphrase luksFormat $part_3
            cryptsetup luksOpen $part_3 root
            pvcreate /dev/mapper/root
            vgcreate gentooVG /dev/mapper/root
            lvcreate -L 32GiB gentooVG -n gentooSwap
            lvcreate -L 64GiB gentooVG -n gentooRoot
            lvcreate -l +100%FREE gentooVG -n gentooHome
            vgchange --available y
            mkswap /dev/mapper/gentooVG-gentooSwap
            mkfs.ext4 /dev/mapper/gentooVG-gentooRoot
            mkfs.ext4 /dev/mapper/gentooVG-gentooHome
            swapon /dev/mapper/gentooVG-gentooSwap
            swap_part=("/dev/mapper/gentooVG-gentooSwap")
            root_part=("/dev/mapper/gentooVG-gentooRoot")
            home_part=("/dev/mapper/gentooVG-gentooHome")
            rm -rf devices
            clear
            sleep 2
            break
...
mv gentooInstall.d /mnt/gentoo
mv gentooInstall.d.zip /mnt/gentoo/
mv network_devices /mnt/gentoo/gentooInstall.d/
cd /mnt/gentoo/gentooInstall.d

install_vars=/mnt/gentoo/gentooInstall.d/install_vars
...
echo "$part_swap" >> "$install_vars"
echo "$part_1" >> "$install_vars"
echo "$part_2" >> "$install_vars"
echo "$part_root" >> "$install_vars"
echo "$part_home" >> "$install_vars"
...
cd /mnt/gentoo/gentooInstall.d/gentoo/
cp -a /mnt/gentoo/gentooInstall.d/gentoo/portage/package.use/. /mnt/gentoo/etc/portage/package.use/
cd /mnt/gentoo/
rm -rf /mnt/gentoo/etc/portage/make.conf
##copies our pre-made make.conf over
cp /mnt/gentoo/gentooInstall.d/gentoo/portage/make.conf /mnt/gentoo/etc/portage/
...
#cp /mnt/gentoo/gentooInstall.d/gentoo/portage/linux_drivers /mnt/gentoo/etc/portage/
cp /mnt/gentoo/gentooInstall.d/gentoo/portage/package.license /mnt/gentoo/etc/portage
#cp /mnt/gentoo/gentooInstall.d/gentoo/portage/package.accept_keywords /mnt/gentoo/etc/portage/
#cp -r /mnt/gentoo/gentooInstall.d/gentoo/portage/profile /mnt/gentoo/etc/portage/
...
cp /mnt/gentoo/gentooInstall.d/post_chroot.sh /mnt/gentoo/
cp /mnt/gentoo/gentooInstall.d/install_vars /mnt/gentoo/
