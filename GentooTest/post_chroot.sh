...
cpus=$(sed '5q;d' install_vars)
part_swap=$(sed '6q;d' install_vars)
part_1=$(sed '7q;d' install_vars)
part_2=$(sed '8q;d' install_vars)
part_root=$(sed '9q;d' install_vars)
part_home=$(sed '10q;d' install_vars)
...
if [ $kernelanswer = "no" ]; then
	#cp /gentooInstall.d/gentoo/kernel/gentoominimal /root/kernel.config
	#genkernel --kernel-config=/root/kernel.config all
	cp /gentooInstall.d/gentoo/kernel/gentoominimal /usr/src/linux/.config
...
UUID2=("UUID=${UUID2}")
UUID3=$(blkid -s UUID -o value $part_swap)
UUID3=("UUID=${UUID3}")
UUID4=$(blkid -s UUID -o value $part_root)
UUID4=("UUID=${UUID4}")
UUID4=$(blkid -s UUID -o value $part_home)
UUID4=("UUID=${UUID5}")
...
printf "%s\t\t/home\t\text4\t\tnoatime\t\t0 1\n" $UUID4 >> /etc/fstab
...
cd $scriptdir
cp doas.conf /etc/
printf "installed doas and enabled it for wheel group\n"
...
printf "added user\n"
cd ..
mv gentooInstall.d.zip /home/$username
##rm -rf /gentooInstall.d
stage3=$(ls stage3*)
rm -rf $stage3
...
rm -rf /install_vars
cp -r /gentooInstall.d/gentoo/portage/savedconfig /etc/portage/
cp -r /gentooInstall.d/gentoo/portage/env /etc/portage/
cp /gentooInstall.d/gentoo/portage/package.env /etc/portage/
rm -rf /gentooInstall.d
